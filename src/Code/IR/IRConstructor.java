...
//        addInst(new Branch(currentLabel, trueLabel, falseLabel, rhs, rhsOp));
    ...
    {
        Address left = (Address) visit(node.getLhs());
        IntegerValue right;
    ...
    private void addBinaryInst(Address dest, IntegerValue left, IntegerValue right, BinaryOp op)
    {
        ...
        else
        {
            ...
            switch (op)
            {
                ...
                case L_SHIFT: binaryOp = BinaryOperation.BinaryOp.sal; break;
                case R_SHIFT: binaryOp = BinaryOperation.BinaryOp.sar; break;
...
//            throw new RuntimeException("cannot be here now");
            ...
            currentFunctionScope.increSlotNumber();
            addInst(new Malloc(currentLabel, new Immediate(node.getExprType().getTypeSize() / 8), address));
            return address;
        }
...
//                address, new Immediate(-1)), size));
        ...
        if(node.size() != 0)
        {
            ...
            addInst(new Branch(currentLabel, trueLabel, falseLabel, compareDest,
                    Compare.Condition.SEQ, false));
            addInst(new Jump(currentLabel, trueLabel));
            addInst(falseLabel);
        }
        return address;
...
//            node.setValue("\n");
        ...
        Address address = new Address(Name.getName(name), new BuiltIn());
        address.setStringConst(true);
        return address;
    }
...
//        return new BasicBlock(block);
    ...
    public IRInstruction visit(ForNode node)
    {
        ...
        if(endCondition != null)
        {
            ...
            addInst(new Branch(currentLabel, trueLabel, falseLabel, endCondition, false));
        }
        else addInst(new Jump(currentLabel, trueLabel));
        addInst(trueLabel);
        visit(node.getBlock());
        addInst(new Jump(currentLabel, jumpLabel));
        addInst(jumpLabel);
    ...
    public IRInstruction visit(IfNode node)
    {
        ...
        addInst(new Branch(currentLabel, trueLabel, falseLabel, condition, false));
        addInst(trueLabel);
        visit(node.getThen());
